...
function RoutineStarter({ userId }: { userId: string }) {
  ...
  async function recolour(routine: { id: string; color: string }) { const value = window.prompt('Routine colour (hex)', routine.color)?.trim(); if (!value || !/^#[0-9a-fA-F]{6}$/.test(value) || !supabase) return setMessage('Use a six-digit hex colour, for example #0D6E5D.'); const { error } = await supabase.from('routines').update({ color: value }).eq('id', routine.id); if (error) setMessage(error.message); else { setRoutines(routines.map((item) => item.id === routine.id ? { ...item, color: value } : item)); setMessage('Routine colour updated.') } }
  async function archive(routineId: string) { if (!window.confirm('Archive this routine? Its history will be preserved.')) return; if (!supabase) return; const { error } = await supabase.from('routines').update({ archived_at: new Date().toISOString() }).eq('id', routineId); if (error) setMessage(error.message); else { setRoutines(routines.filter((item) => item.id !== routineId)); setMessage('Routine archived.') } }
  async function moveRoutine(id: string, direction: -1 | 1) { const index = routines.findIndex((item) => item.id === id); const target = index + direction; if (!supabase || target < 0 || target >= routines.length) return; const next = [...routines]; [next[index], next[target]] = [next[target], next[index]]; const normalized = next.map((item, order) => ({ ...item, sort_order: order })); setRoutines(normalized); const { error } = await supabase.from('routines').upsert(normalized.map(({ id: routineId, sort_order }) => ({ id: routineId, user_id: userId, sort_order })), { onConflict: 'id' }); if (error) { setRoutines(routines); setMessage(error.message) } else setMessage('Routine order saved.') }
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">ROUTINE TRACKER</p><h1>{weekOffset === 0 ? 'This week' : week[0].toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</h1><p className="muted">{week[0].toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</p></div><button className="today-button" onClick={() => setWeekOffset(0)}>Today</button></div>
    <div className="week-nav"><button aria-label="Previous week" onClick={() => setWeekOffset(weekOffset - 1)}><AppIcon name="arrow" /></button><span>{shortDate(week[0])} – {shortDate(week[6])}</span><button aria-label="Next week" className="next" onClick={() => setWeekOffset(weekOffset + 1)}><AppIcon name="arrow" /></button></div>
    <section className="module-card tracker-card">
      <div className="section-title"><h2>Your routines</h2><span>{visible.length} active</span></div><div className="search-bar"><input aria-label="Search routines" placeholder="Search routines" value={search} onChange={(event) => setSearch(event.target.value)} /></div>
      {visible.length === 0 ? <div className="empty-state"><div className="empty-icon"><AppIcon name="grid" /></div><h3>{search ? 'No matching routines' : 'Start your rhythm'}</h3><p>{search ? 'Try another routine name.' : 'Add a routine you want to practise. Your weekly progress will appear here.'}</p></div> : <div className="routine-grid"><div className="grid-head"><span>Task</span>{week.map((day) => <span key={dateKey(day)} className={dateKey(day) === dateKey(today) ? 'today-label' : ''}>{dayLabel(day)}<b>{day.getDate()}</b></span>)}<span>Total</span></div>{visible.map((routine) => { const row = completions.filter((item) => item.routine_id === routine.id).sort((a, b) => a.completed_on.localeCompare(b.completed_on)); return <div className="grid-row" key={routine.id}><div className="routine-cell"><button className="routine-name" onClick={() => rename(routine)} title="Tap to rename"><i style={{ background: routine.color }} />{routine.name}</button><button className="colour-edit" onClick={() => recolour(routine)} aria-label={`Change ${routine.name} colour`}>●</button><button className="order-edit" onClick={() => moveRoutine(routine.id, -1)} aria-label={`Move ${routine.name} up`}>↑</button><button className="order-edit" onClick={() => moveRoutine(routine.id, 1)} aria-label={`Move ${routine.name} down`}>↓</button></div>{week.map((day) => { const key = dateKey(day); const index = row.findIndex((item) => item.completed_on === key); const disabled = key < routine.started_on; return <button disabled={disabled} onClick={() => toggle(routine.id, key)} className={`completion ${index >= 0 ? 'done' : ''}`} style={index >= 0 ? { background: routine.color } : undefined} key={key} aria-label={`${routine.name}, ${shortDate(day)}`}>{index >= 0 ? index + 1 : ''}</button> })}<button className="row-total" onClick={() => archive(routine.id)} title="Archive routine">{row.length}</button></div>})}</div>}
      <form className="inline-add" onSubmit={addRoutine}><input aria-label="New routine name" placeholder="Add a routine…" value={name} onChange={(event) => setName(event.target.value)} /><button aria-label="Add routine" type="submit"><AppIcon name="plus" /></button></form>
      {message && <p className="form-message" role="status">{message}</p>}
    </section>
  </>
}
